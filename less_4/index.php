...
<h1>Gallery</h1>
<?php
// Проверяем наличие нужных каталогов. При отсутствии - создаем с привилегиями
...
$dir = "gallery/";
$dirtmp = "gallery_tmp/";
// Если каталог создан, ищем и выводим на страницу изображения из него
if (file_exists($dir)) {
	$files = scandir($dir);
...
if(isset($_POST['upload'])) {
	...
	foreach ($_FILES[userfile][name] as $key => $fname) {
		$info = pathinfo($fname); // Проверяем тип файла и его размер
		if ((($info['extension'] == "jpg") || ($info['extension'] == "jpeg") || ($info['extension'] == "gif") || ($info['extension'] == "png")) && (filesize($_FILES[userfile][tmp_name][$key]) <= 2048000)) {
			if(copy($_FILES[userfile][tmp_name][$key],$dir.$_FILES[userfile][name][$key])) // Подгоняем размер под шаблон и сохраняем уменьшенную копию
			{
